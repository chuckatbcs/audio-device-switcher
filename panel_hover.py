...
class PanelHoverTracker:
    ...
    icon hotspot. When the pointer dwells near that spot on the panel strip,
    show the status popup — the closest we can get to hover without panel support.
    """

    PANEL_STRIP_PX = 56
    HOTSPOT_RADIUS_PX = 36
    DWELL_MS = 450
    POLL_MS = 150

    def __init__(self, show_callback, hide_callback):
    ...
    def _poll(self):
        ...
        on_panel_strip = y >= geom.y + geom.height - self.PANEL_STRIP_PX
